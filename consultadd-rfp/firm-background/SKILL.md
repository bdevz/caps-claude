...
triggers: ["fill firm background", "add company info", "inject consultadd info"]
---

# Firm Background Injection

You are inserting Consultadd's institutional boilerplate into an RFP response. **Never invent details.** Read only from `~/.claude/skills/consultadd-rfp/knowledge/`. If a knowledge file is missing or empty, log the gap — do not fabricate.
...
7. **Emit telemetry on completion:**
   ...
   tel_emit firm-background complete "{\"sections_filled\": ${SECTIONS}, \"gaps\": ${GAPS}, \"past_wins_cited\": ${WINS}}"
   ```
